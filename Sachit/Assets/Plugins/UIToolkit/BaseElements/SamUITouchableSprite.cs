...
public abstract class SamUITouchableSprite : SamUISprite, ITouchable, IComparable
{
	public int touchCount;
	public SamUIUVRect disabledUVframe; // when disabled, this UV frame will be used if it is set
	public SamUIUVRect hoveredUVframe; // when hovered over, this UV frame will be used if it is set
	
	protected SamUIEdgeOffsets _normalTouchOffsets;
	protected SamUIEdgeOffsets _highlightedTouchOffsets;
	protected Rect _highlightedTouchFrame;
	protected Rect _normalTouchFrame;
	protected SamUIUVRect _tempUVframe; // Holds a copy of the uvFrame while in either the highlighted or disabled state
	...
	public SamUITouchableSprite( Rect frame, int depth, SamUIUVRect uvFrame ):base( frame, depth, uvFrame )
	{
		_tempUVframe = uvFrame;
	}
	
	
	// constructor for when the need to have a centered SamUISprite arises (I'm looking at you SamUIKnob)
	public SamUITouchableSprite( Rect frame, int depth, SamUIUVRect uvFrame, bool gameObjectOriginInCenter ):base( frame, depth, uvFrame, gameObjectOriginInCenter )
	{
	}
	...
	public SamUIEdgeOffsets highlightedTouchOffsets
	{
	...
	// Adds or subtracts from the frame of the button to define a hit area
	public SamUIEdgeOffsets normalTouchOffsets
	{
	...
	private Rect addOffsetsAndClipToScreen( Rect frame, SamUIEdgeOffsets offsets )
	{
	...
	public virtual bool hoveredOver
	{
		...
		set
		{
			...
			if( hoveredUVframe.Equals( SamUIUVRect.zero ) )
				return;
	...
	// a disabled SamUITouchableSprite will have a touchFrame of all zeros
	public virtual bool disabled
	{
		...
		set
		{
			if( _disabled != value )
			{
				...
				if( value && !disabledUVframe.Equals( SamUIUVRect.zero ) )
