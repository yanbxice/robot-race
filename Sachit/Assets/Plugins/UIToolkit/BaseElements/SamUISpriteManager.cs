...
public struct SamUITextureInfo
{
	public SamUIUVRect uvRect;
	public Rect frame;
	public Rect sourceSize;
}


public class SamUISpriteManager : MonoBehaviour 
{
	#region ivars
	
	public enum SamUISpriteWindowOrder
	{
	    CCW,
	    CW
	};
	...
	private SamUISprite[] _sprites = new SamUISprite[0];    // Array of all sprites (the offset of the vertices corresponding to each sprite should be found simply by taking the sprite's index * 4 (4 verts per sprite).
	private SamUISpriteWindowOrder winding = SamUISpriteWindowOrder.CCW; // Which way to wind polygons
	...
	protected Dictionary<string, SamUITextureInfo> textureDetails; // texture details loaded from the TexturePacker config file
	...
	public void loadTextureAndPrepareForUse()
	{
		...
		if( SamUI.instance.isHD )
			texturePackerConfigName = texturePackerConfigName + SamUI.instance.hdExtension;
		
		var texture = (Texture)Resources.Load( texturePackerConfigName, typeof( Texture ) );
		if( texture == null )
			Debug.Log( "SamUI texture is being autoloaded and it doesnt exist.  Cannot find texturePackerConfigName: " + texturePackerConfigName );
	...
	private Dictionary<string, SamUITextureInfo> loadTexturesFromTexturePackerJSON( string filename )
	{
		var textures = new Dictionary<string, SamUITextureInfo>();
		...
		foreach( System.Collections.DictionaryEntry item in frames )
		{
			...
			var ti = new SamUITextureInfo();
			ti.frame = new Rect( frameX, frameY, frameW, frameH );
			ti.uvRect = new SamUIUVRect( frameX, frameY, frameW, frameH, textureSize );
	...
	/// <summary>
	/// grabs the SamUITextureInfo for the given filename
	/// </summary>
	public SamUITextureInfo textureInfoForFilename( string filename )
	{
#if UNITY_EDITOR
	...
	/// </summary>
	public SamUIUVRect uvRectForFilename( string filename )
	{
#if UNITY_EDITOR
		// sanity check while in editor
		if( !textureDetails.ContainsKey( filename ) )
    ...
    protected int expandMaxSpriteLimit( int count )
    {
        ...
        // Resize sprite array:
        SamUISprite[] tempSprites = _sprites;
        _sprites = new SamUISprite[_sprites.Length + count];
        ...
        for( int i = firstNewElement; i < _sprites.Length; ++i )
        {
            ...
            if( winding == SamUISpriteWindowOrder.CCW )
	...
	/// </summary>
	public SamUISprite addSprite( string name, int xPos, int yPos )
	{
		return addSprite( name, xPos, yPos, 1, false );
	}
	
	
	public SamUISprite addSprite( string name, int xPos, int yPos, int depth )
	{
		return addSprite( name, xPos, yPos, depth, false );
	}

	
    public SamUISprite addSprite( string name, int xPos, int yPos, int depth, bool gameObjectOriginInCenter )
    {
#if UNITY_EDITOR
		// sanity check while in editor
	...
	/// </summary>
    private SamUISprite addSprite( Rect frame, SamUIUVRect uvFrame, int depth, bool gameObjectOriginInCenter )
    {
        // Create and initialize the new sprite
		SamUISprite newSprite = new SamUISprite( frame, depth, uvFrame, gameObjectOriginInCenter );
		addSprite( newSprite );
	...
	/// </summary>
    public void addSprite( SamUISprite sprite )
    {
        ...
        sprite.index = i;
        sprite.manager = this as SamUIToolkit;
    ...
    protected void removeSprite( SamUISprite sprite )
    {
    ...
    public void hideSprite( SamUISprite sprite )
    {
    ...
    public void showSprite( SamUISprite sprite )
    {
        if( !sprite.___hidden )
            return;
    ...
    public void updateUV( SamUISprite sprite )
    {
    ...
    public void updateColors( SamUISprite sprite )
    {
