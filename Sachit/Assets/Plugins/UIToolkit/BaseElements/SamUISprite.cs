...
public class SamUISprite : SamUIObject, IPositionable
{
    public SamUIToolkit manager = null; // Reference to the sprite manager in which this sprite resides
	...
	public SamUIVertexIndices vertexIndices;
	...
	protected SamUIUVRect _uvFrame; // UV coordinates and size for the sprite
	protected SamUIUVRect _uvFrameClipped; // alternate UV coordinatest for when a sprite it clipped
	...
	protected Dictionary<string, SamUISpriteAnimation> spriteAnimations;
	
	
	public SamUISprite( Rect frame, int depth, SamUIUVRect uvFrame ) : this( frame, depth, uvFrame, false )
	{}
	

    public SamUISprite( Rect frame, int depth, SamUIUVRect uvFrame, bool gameObjectOriginInCenter ) : base()
    {
	...
	/// </summary>
	public virtual SamUIUVRect uvFrame
	{
	...
	/// <summary>
	/// Alternate uvFrame for when the sprite is clipped. Set to SamUIUVRect.zero to remove clipping or set
	/// clipped to false.
	/// Note: setting this automatically sets the sprite as clipped
	/// </summary>
	public virtual SamUIUVRect uvFrameClipped
	{
		...
		set
		{
			...
			_clipped = value != SamUIUVRect.zero;
			manager.updateUV( this );
		}
	}
    ...
    // Sets the specified color and automatically notifies the GSamUISpriteManager to update the colors
	...
	public SamUISpriteAnimation addSpriteAnimation( string name, float frameTime, params string[] filenames )
	{
		// create the spriteAnimations dictionary on demand
		if( spriteAnimations == null )
			spriteAnimations = new Dictionary<string, SamUISpriteAnimation>();
		
		// get the SamUIUVRects for the sprite frames
		var uvRects = new List<SamUIUVRect>( filenames.Length );
		
		foreach( var filename in filenames )
			uvRects.Add( this.manager.uvRectForFilename( filename ) );
		
		var anim = new SamUISpriteAnimation( frameTime, uvRects );
	...
	public void playSpriteAnimation( SamUISpriteAnimation anim, int loopCount )
	{
		SamUI.instance.StartCoroutine( anim.play( this, loopCount ) );
	}
