...
public class SamUI : MonoBehaviour
{
	// All access should go through instance and you are trusted not to set it :)
	static public SamUI instance = null;
	static public SamUIToolkit firstToolkit = null; // we stick the first available toolkit so when using one atlas the API remains simple
	
	public bool scaleHUD = true;
	
	public int drawDepth = 100;	
	public LayerMask SamUILayer = 0;
	[HideInInspector]
	public int layer;
	...
	private SamUIToolkit[] _toolkitInstances;
	...
	private void Awake()
	{
		...
		if (scaleHUD)
		{
			_uiCamera.transform.position = new Vector3( 240, -160, -10.0f );
			_uiCamera.orthographicSize = 160;
		}
		else
		{
		...
		_uiCamera.cullingMask = SamUILayer;
		
		// Cache the layer for later use when adding sprites
		// SamUILayer.value is a mask, find which bit is set 
		for( int i = 0; i < sizeof( int ) * 8; i++ )
		{
			if( ( SamUILayer.value & (1 << i) ) == (1 << i) )
			{
				layer = i;
				break;
			}
		...
		if( autoTextureSelectionForHD && deviceAllowsHD )
		{
			...
			{
#if UNITY_EDITOR
				Debug.Log( "switching to 2x GSamUI texture" );
#endif
				isHD = true;
			}
		}
		
		// grab all our child SamUIToolkits
		_toolkitInstances = GetComponentsInChildren<SamUIToolkit>();
		firstToolkit = _toolkitInstances[0];
#if UNITY_EDITOR
		if( _toolkitInstances.Length == 0 )
			throw new System.Exception( "Could not find any SamUIToolkit instances in children of SamUI" );
#endif
		
		// kick off the loading of texture for any SamUIToolkits we have
